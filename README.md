...
**Свой VPN целиком: протокол, ноды, панель и приложения.** Одной системой,
а не связкой из четырёх чужих проектов — и для того, кто раздаёт доступ, и
для того, кто просто подключается.
...
**Чего пока нет:** iOS, отзыва одного устройства из нескольких (у них общий
ключ, различить их нечем), языков кроме русского и английского. Версия начинается с нуля осознанно: до 1.0 API и форматы могут
меняться.
