...
// Оповещения продавцу.
//
// Про свои беды продавец узнаёт последним: нода молчит третий час, у двадцати
// человек кончается срок, копия базы не снялась неделю назад. Сегодня он
// узнаёт это из жалоб покупателей — то есть тогда, когда чинить уже поздно и
// деньги уже потеряны.
//
// Шлём в его собственный телеграм-бот: у него он уже есть, покупатели и так
// пишут туда, и заводить ради оповещений что-то ещё незачем.
//
// Что в сообщениях НЕ появляется: имена, метки и идентификаторы покупателей.
// «У троих кончается срок», а не «у Артёма, Нины и Дмитрия». Переписка с ботом
// живёт в телеграме, и превращать её в список клиентов продавца нельзя: она
// переживёт и панель, и его самого. Числа отвечают на вопрос «надо ли идти
// смотреть» ничем не хуже, а кто именно — видно в панели.
//
// Чем платим: панель начинает ходить наружу, на api.telegram.org. Её адрес
// становится известен телеграму и виден тому, кто смотрит на её трафик. Для
// панели, которая и так стоит на домене продавца, это приемлемо, но выбор
// осознанный — поэтому оповещения выключены, пока продавец их не включит.
...
// нужен, а беда важная — продавец узнаёт о несделанных копиях обычно в тот
// день, когда они понадобились.
