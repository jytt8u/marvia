...
const (
	...
	// MaxPayload — сколько полезных данных влезает в кадр, если к нему
	// придётся добавить самую длинную добивку.
	MaxPayload = MaxPlaintext - frameHeaderLen - maxFramePad

	// BulkPayload — сколько полезных данных влезает в кадр без добивки.
	//
	// Крупные кадры не добиваются вовсе (см. framePad), и резервировать в
	// них килобайт под добивку значило бы резать скачивание на кадры по
	// 15 КиБ там, где TLS шлёт ровно по 16: и кадров на 7 % больше, и
	// размер не как у всех. Кадр в 16 384 байта содержимого — это TLS-рекорд
	// во всю длину, самое обычное, что бывает в потоке скачивания.
	BulkPayload = MaxPlaintext - frameHeaderLen
